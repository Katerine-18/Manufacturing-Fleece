...
Public Class AgregarLote
    Private Sub AgregarLote_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        LlenarCmbColores()
        ' Se eliminó llenarDatagrid() porque el grid está en DataReporte
    End Sub
        ...
        Try
            Using con As New SqlConnection(My.Settings.FLEECE)
                ...
                If Decimal.TryParse(txttotaldzrecibidas.Text, totalRecibidas) Then
                    cmd.Parameters.Add("@TotalDZRecibidas", SqlDbType.Decimal).Value = totalRecibidas
                Else
                    cmd.Parameters.Add("@TotalDZRecibidas", SqlDbType.Decimal).Value = 0
                End If
    ...
    Private Sub btneliminar_Click(sender As Object, e As EventArgs) Handles btneliminar.Click
        ...
        Dim respuesta As DialogResult = MessageBox.Show(
            "¿Está seguro de eliminar el Lote N# " & txtnum.Text & "? Esta acción no se puede deshacer.",
        ...
        If respuesta = DialogResult.Yes Then
            Try
                Using con As New SqlConnection(My.Settings.FLEECE)
                    con.Open()
                    Dim cmd As New SqlCommand("DELETE FROM AGREGARLOTES WHERE Num = @Num", con)
                    cmd.Parameters.Add("@Num", SqlDbType.Int).Value = Convert.ToInt32(txtnum.Text)

                    Dim filasAfectadas As Integer = cmd.ExecuteNonQuery()

                    If filasAfectadas > 0 Then
                        MessageBox.Show("Lote eliminado satisfactoriamente.", "Éxito", MessageBoxButtons.OK, MessageBoxIcon.Information)
                        DataReporte.llenarDatagrid()  ' Refresca el grid en DataReporte
                        LimpiarTextos()
                    Else
